---
title: Managing Apps
description: Create and manage apps, canvases, consoles, staging, and files via the CLI.
---
...
superplane apps canvas update -f my_canvas.yaml --message "Add HTTP keepalive node"
```

`superplane apps canvas update` stages the file and commits immediately. Pass `--message` (or `-m`) with every
direct update. To stage changes without committing, use `superplane apps staging` instead (see below).
...
Use `superplane apps staging` when you want to stage one or more files and commit them together — the same
stage-and-commit loop as the UI and agent.

```sh
# List staged paths for an app
superplane apps staging status <app_name_or_id>

# Stage local files (mapped to repository paths by filename)
superplane apps staging update \
  --file canvas.yaml \
  --file console.yaml \
  --file README.md

# Commit staged changes to the live app
superplane apps staging commit --message "Add incident routing path"
```

If you already set an active app with `superplane apps active <app_id>`, omit `<app_name_or_id>` on staging
commands.

Each `--file` path maps to a repository path using the file name only. For example, `./tmp/canvas.yaml` stages
as `canvas.yaml`.

You can view a visual diff of staged changes and commit or discard them in the SuperPlane UI as well.
...
# Replace the console from a file (commits immediately)
superplane apps console set <app_name_or_id> console.yaml --message "Add deployment KPI panel"

# Or stage and commit separately
superplane apps staging update --file console.yaml
superplane apps staging commit --message "Add deployment KPI panel"
```
...
```sh
superplane apps console set --file console.yaml --message "Add deployment KPI panel"
```
