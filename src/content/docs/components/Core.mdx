...
The For Each component reads an array from the upstream payload and emits one downstream event per element. See [Looping](/concepts/looping) for patterns and nesting.
...
The Loop component runs downstream steps repeatedly until an exit condition becomes true. See [Looping](/concepts/looping) for patterns and nesting.
