...
<CardGrid>
  <LinkCard
    title="On Compute Instance Created"
    href="#on-compute-instance-created"
    description="Start a workflow when an OCI Compute instance launch completes"
  />
</CardGrid>
...
<CardGrid>
  <LinkCard
    ...
    href="#create-compute-instance"
    description="Provision an OCI Compute instance and wait until it is running"
  />
</CardGrid>
...
The On Compute Instance Created trigger starts a workflow execution whenever an OCI Compute instance
launch completes.

### How It Works

When the OCI integration is set up, SuperPlane automatically creates a shared **OCI Notifications (ONS)
topic** and subscribes to it. When this trigger is added to a workflow, SuperPlane automatically creates an
**OCI Events rule** in the configured compartment that forwards
`com.oraclecloud.computeapi.launchinstance.end` events to that topic — no manual OCI configuration is
required.
...
```json
{
  "data": {
    ...
    "data": {
      "additionalDetails": {
        "imageId": "ocid1.image.oc1.eu-frankfurt-1.aaaaaaaExample",
        "shape": "VM.Standard.E2.1.Micro",
        "type": "CustomerVmi"
      },
      "availabilityDomain": "EXAMPLE:EU-FRANKFURT-1-AD-1",
      "compartmentId": "ocid1.tenancy.oc1..aaaaaaaExample",
      "compartmentName": "root",
      "definedTags": {},
      "freeformTags": {},
      "resourceId": "ocid1.instance.oc1.eu-frankfurt-1.aaaaaaaExample",
...
The Create Compute Instance component provisions a new Oracle Cloud Infrastructure Compute instance and
waits until it reaches **RUNNING** state before emitting.
...
```json
{
  "data": {
    ...
    "instanceId": "ocid1.instance.oc1.eu-frankfurt-1.aaaaExample",
