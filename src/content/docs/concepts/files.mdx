...
To use a repository script in an SSH Command node, set **Command source** to **From file** and point **Command file** at the script's path. The script is re-read from the repository on every run, so any edits take effect on the next run after you publish. The file always runs under bash, regardless of its shebang.
