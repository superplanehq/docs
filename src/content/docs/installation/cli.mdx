...
Or download a binary directly from `install.superplane.com` for specific OS and processor architecture:

```
curl -fsSL https://install.superplane.com/superplane-cli-<os>-<arch> -o superplane
chmod +x superplane
mv superplane ~/.local/bin/superplane
```

Or download a specific version:

```
curl -fsSL https://install.superplane.com/v0.1.6/superplane-cli-<os>-<arch> -o superplane
chmod +x superplane
mv superplane ~/.local/bin/superplane
```
...
version, run `install.sh` again or replace the binary using those URLs.
