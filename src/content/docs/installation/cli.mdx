...
- `true`: versioning enabled. Use `superplane canvases update --draft -f ...` and then `publish`.
- `false`: versioning disabled. Use `superplane canvases update -f ...` (no `--draft`), and do not use `publish`.
- `null`: CLI could not determine it from organization metadata.

Quick behavior-based check:

- If `superplane canvases update --draft ...` returns `--draft cannot be used when canvas versioning is disabled`,
  versioning is disabled.
- If `superplane canvases update ...` (without `--draft`) returns
  `canvas versioning is enabled for this organization; use --draft ...`,
  versioning is enabled.
- If `superplane canvases publish ...` returns `canvas versioning is disabled for this organization`,
  versioning is disabled.
...
- If canvas versioning is disabled, do not use `--draft`; updates apply directly and `publish` is not available.
